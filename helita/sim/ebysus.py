...
def diper2eb_atom_ascii(atomfile,output):
    ...
    text = ["#        E[cm-1]     g              label[35]                   stg  lvlN \n"  +
            "#                        '----|----|----|----|----|----|----|'\n"]
    ...
    for iv in range(8,8+nk):
        ...
        data[iv] = ('    {0:13.3f}'.format(float(line[0])) + '  {0:5.2f}'.format(float(line[1])) + " ' {0:2}".format(atom.upper()) + ' {0:5}'.format(num2roman(int(line[-1]))) +
                ' {0:26}'.format(strlvl[0]) + "'  {0:3d}".format(int(line[-1])) + '   {0:3d}'.format(iv-7)+ '\n') # the two iv are wrong at the end...
