...
int ck_synch_hd(FILE *fin,struct fzhead *fh,int t_endian)
{
  int wwflag=0;
  if(fread(fh,1,sizeof(struct fzhead),fin)!=sizeof(struct fzhead)){
...
char *ana_fzhead(char *file_name) // fzhead subroutine
{
  ...
  fprintf(stdout,"reading ANA file \"%s\" header\n",file_name);
  int one=1;
...
uint8_t *ana_fzread(char *file_name,int **ds,int *nd,char **header,int *type,int *osz) // fzread subroutine
{
  ...
  fprintf(stdout,"reading ANA file \"%s\"\n",file_name);
...
//
  ...
  *ds=((int*)malloc(sizeof(int)*(*nd=fh.ndim)))-1;
  int d;
  for(d=1;d<=*nd;++d) (*ds)[d]=fh.dim[d-1];
...
// read data
	...
	// Allocate 4 bytes extra to solve the possible illegal read beyond the
	// malloc'ed memory in anadecrunch() functions.
...
void ana_fzwrite(uint8_t *data,char *file_name,int *ds,int nd,char *header,int type)
...
//
  int n_elem=1,n;
  for(n=1;n<=nd;++n) n_elem*=(fh.dim[n-1]=ds[n]);
  int nx=fh.dim[0];
  int ny=n_elem/nx;
  int type_sizes[]=ANA_VAR_SZ;
  int size=n_elem*type_sizes[type];
...
/* extended to 32 bits 2/4/96 */
  ...
  int i;
  for(i=0;i<=3;++i) fh.cbytes[i]=((uint8_t*)&res)[i];
  fwrite(&fh,1,sizeof(struct fzhead),f);  // write header
  fwrite(q,1,size,f);
  free(q);
  fclose(f);
}
